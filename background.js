...
const data = { model: 'llama-3.3-70b-versatile', messages: [{ role: 'user', content: 'This is a test prompt, please reply with: passed' }] };
...
const tabLooping = () => {
  ...
  }, 1000); // Be mindful that this may be an issue later on (D.O.)
};
...
const handleTabGrouping = async (tab) => {
  ...
  try {
    ...
    let existingGroupId = null;
      ...
      --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
      getGroqResponse();
      console.log("title is: " + tab.title { autoGroups } ); // tab title will be more useful than the url for the LLM to identify the group
