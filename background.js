...
const apiKey = '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';
...
const data = {
  ...
  messages: [
    {
      role: 'user',
      content: 'This is a test prompt, please reply with: passed',
    },
  ],
};
  ...
  fetch(url, {
    ...
    .then((result) => {
      ...
      if (result.choices && result.choices.length > 0) {
        ...
        console.error(
          'Error: Invalid response format. "choices" may be undefined or empty.'
        );
      }
    })
    .catch((error) => {
...
// timing related variables (all in Unix time or in ms)
let lastGroqCalledTime = 0; // stores the last called time (Unix time)
let initialDelay = 25; // 25ms -> default
let mainTabLoopDelay = 1000; // 1000ms -> default
let groqCallDelay = 5000; // 5000ms = 5s
...
chrome.storage.onChanged.addListener(async (changes) => {
  ...
  getGroqResponse();
  //fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify(data) }).then(response => response.json()).then(result => { const responseText = result.choices[0].message.content; console.log(responseText); }).catch(error => { console.error('Error:', error); });
});
...
const tabLooping = () => {
  ...
  }, mainTabLoopDelay); // Be mindful that this may be an issue later on (D.O.)
};
...
const handleTabGrouping = async (tab) => {
  ...
  while (isGrouping) {
    await new Promise((resolve) => setTimeout(resolve, initialDelay)); // Polling with a short delay
  }
  ...
  try {
    ...
    const inAutoGroup = autoGroups.some(
      (group) => tab.groupId === group.idInChrome
    ); // Will be important later on (D.O.)
    ...
    /* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- //
      ...
      --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    // additional code to limit Groq calls (modify as necessary)
    const currentTime = Date.now(); // Get the current time in milliseconds
    const timeSinceLastCall = currentTime - lastCalledTime;

    if (timeSinceLastCall >= groqCallDelay) {
      // Check if requisite amt of time have passed
      getGroqResponse(); // Call the function
      lastCalledTime = currentTime; // Update the last called time
    }
    console.log('title is: ' + tab.title); // tab title will be more useful than the url for the LLM to identify the group
  ...
  try {
    ...
    for (const group of autoGroups) {
      ...
      for (const tab of tabsInGroup) {
        ...
        console.log(
          `Ungrouped tab with ID: ${tab.id} from group ID: ${group.idInChrome}`
        );
      }
    }
